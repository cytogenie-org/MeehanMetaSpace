 ...
 */
public class DnD2 extends TransferHandler implements Serializable {
	...
	// --- DropTargetListener methods (multicast) --------------------------
		...
		public synchronized  void dropActionChanged(DropTargetDragEvent e) {
	...
	// --- DropTargetListener methods -----------------------------------
		...
		DnD2 importer = (DnD2)c.getTransferHandler();
	...
	public void dragGestureRecognized(DragGestureEvent dge){
	  ...
	  DnD2 th=(DnD2)c.getTransferHandler();
	  ...
	  if (t != null){
		...
		try{
		  ...
		  if (img == null){
			...
			System.out.println("invoking startDrag with *THE* icon "+icn+" for " + t);
			final Cursor cur=Toolkit.getDefaultToolkit().createCustomCursor(img, new Point(5,5), "custom");
			dge.startDrag(null, img, new Point(5,5), t, this);
		  }
	 ...
	 */
		...
		if (dsde.getDropSuccess()) {
				( (DnD2)c.getTransferHandler()).exportDone(c, dsc.getTransferable(), dsde.getDropAction());
		} else {
				( (DnD2)c.getTransferHandler()).exportDone(c, null, NONE);
			}
			c.setAutoscrolls(scrolls);
	}
