 ...
 * <code>stop()</code> and <code>interrupt()</code> methods. <br />
 * <br />
 * Example: <br />
 * <br />
 * 
...
public class InfiniteProgressPanel extends JComponent implements MouseListener {
	...
	protected int barsCount = DEFAULT_BAR_COUNT;
	public static int DEFAULT_BAR_COUNT=12;
	/** Amount of frames per seconde. Lowers this to save CPU. */
	protected float framesPerSecond = DEFAULT_FRAMES_PER_SECOND;
	public static float DEFAULT_FRAMES_PER_SECOND=3.0f;
	...
	public InfiniteProgressPanel(String text, int barsCount, float shield,
		...
		this(text, barsCount, shield, framesPerSecond, 300);
	}
	 ...
	 */
	public InfiniteProgressPanel(String text, int barsCount, float shield,
			float framesPerSecond, int rampDelay) {
		this._description = text;
		this.rampDelay = rampDelay >= 0 ? rampDelay : 0;
		this.shield = shield >= 0.0f ? shield : 0.0f;
		this.framesPerSecond = framesPerSecond > 0.0f ? framesPerSecond : DEFAULT_FRAMES_PER_SECOND;
	...
	public synchronized void start(final String description) {
		start(null, description);
	}
	 ...
	 */
	public synchronized void start(final Runnable operation, final String description) {
		if (operation !=null){
			System.out.println("Setting op to "+operation);
	...
	public synchronized void stop() {
		runs--;
		if (runs<0){
			new Exception("Ouch more calls to stop than start!").printStackTrace();
			runs=0;
		} else 	if (runs == 0) {
			if (animation != null) {
				animation.interrupt();
	...
	public void paintComponent(Graphics g) {
		if (started) {
			...
			if (_description != null && _description.length() > 0) {
				...
				TextLayout layout = new TextLayout(_description, getFont(), context);
	...
	private class Animator implements Runnable {
		...
		public void run() {
			...
			while (!Thread.interrupted()) {
				...
				try {
					Thread.sleep(inRamp ? 10 : (int) (1000 / framesPerSecond));
				} catch (InterruptedException ie) {
					break;
				}
				Thread.yield();
				if (_operation != null) {
					if (loops < swingEventsBeforeStartingExpensiveOperation) {
						try {
						...
						final Runnable op = _operation;
						_operation = null;
						SwingUtilities.invokeLater(new Runnable() {
							public void run() {
								Thread.yield();
								PopupBasics.forceBottomRight = true;
								op.run();
								stop();
	...
	private Runnable _operation=null;
	private static final int DEFAULT_SWING_EVENTS_BEFORE_STARTING_EXPENSIVE_OPERATION=6;
	private int swingEventsBeforeStartingExpensiveOperation=DEFAULT_SWING_EVENTS_BEFORE_STARTING_EXPENSIVE_OPERATION;
	public void setSwingEventsBeforeStartingExpensiveOperation(int n){
		swingEventsBeforeStartingExpensiveOperation=n;
	}

	public void run(final Runnable operation, final String description) {
		run(operation, DEFAULT_SWING_EVENTS_BEFORE_STARTING_EXPENSIVE_OPERATION, description);
	}
	
	public void run(final Runnable operation, final int swingEventsBeforeStartingExpensiveOperation, final String description) {
		this.swingEventsBeforeStartingExpensiveOperation=swingEventsBeforeStartingExpensiveOperation;
